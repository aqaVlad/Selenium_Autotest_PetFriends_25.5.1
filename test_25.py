...
class TestPF:
    ...
    """см. комментарий ниже"""
    # def test_all_names_different_1(self, setup):
    #     browser_options = webdriver.EdgeOptions()
    #     driver = webdriver.Edge(options=browser_options)
    ...
    """данный тест на примере кода из модуля и из предложенного ментором в комментариях
    # проходит при любых данных (прогоняет пустым списком и проходит при противоположных ожиданиях).
    # возможно сначала нужно где-то агрегировать собранные данные,
    # но и при создании списка результат тот же [см. следующий тест]"""
    #
    #
    # def test_all_names_different_2(self, setup):
    #     browser_options = webdriver.EdgeOptions()
    #     driver = webdriver.Edge(options=browser_options)
    #     list_names = []
    #     list_images = []
